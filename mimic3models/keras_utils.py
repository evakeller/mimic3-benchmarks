...
class InHospitalMortalityMetrics(keras.callbacks.Callback):
    def __init__(
        self, val_data, target_repl, batch_size=32, early_stopping=True, verbose=2
    ):
    ...
    def calc_metrics(self, data, history, dataset, logs):
        x = np.array(data[0]).astype(np.float32)
        if self.target_repl:
            y_true = np.array(data[1][0]).flatten()
        else:
            y_true = np.array(data[1]).flatten()

        if self.target_repl:
            outputs = self.model.predict(x, batch_size=self.batch_size)
            predictions = outputs[0].flatten()
        else:
            predictions = self.model.predict(x, batch_size=self.batch_size).flatten()
        print("\n")

        predictions = np.stack([1 - predictions, predictions], axis=1)
        ret = metrics.print_metrics_binary(y_true, predictions)
        for k, v in ret.items():
            logs[f"{dataset}_{k}"] = v
        history.append(ret)

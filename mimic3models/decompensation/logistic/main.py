...
def main():
    ...
    for penalty, C in zip(penalties, coefs):
        ...
        save_results(
            train_names,
            train_ts,
            logreg.predict_proba(train_X)[:, 1],
            train_y,
            os.path.join(args.output_dir, "train_predictions", file_name + ".csv"),
        )
        save_results(
            test_names,
            test_ts,
            prediction,
            test_y,
            os.path.join(args.output_dir, "test_predictions", file_name + ".csv"),
        )
        save_results(
            val_names,
            val_ts,
            logreg.predict_proba(val_X)[:, 1],
            val_y,
            os.path.join(args.output_dir, "val_predictions", file_name + ".csv"),
        )

    ...
    for penalty, C in zip(penalties, coefs):
        ...
        for task_id in range(n_tasks):
            ...
            logreg = LogisticRegression(
                penalty=penalty, C=C, solver=solver, random_state=42
            )
        ...
        save_results(
            ...
            train_activations,
            train_y,
            os.path.join(args.output_dir, "train_predictions", model_name + ".csv"),
        )
        save_results(
            test_names,
            test_ts,
            test_activations,
            test_y,
            os.path.join(args.output_dir, "test_predictions", model_name + ".csv"),
        )
        save_results(
            val_names,
            val_ts,
            val_activations,
            val_y,
            os.path.join(args.output_dir, "val_predictions", model_name + ".csv"),
        )

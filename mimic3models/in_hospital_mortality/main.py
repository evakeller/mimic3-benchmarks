...
optimizer_config = {
    ...
    "config": {"learning_rate": args.lr, "beta_1": args.beta_1},
}
...
# Debugging statements to check dataset size and batch size
print(f"Training data shape: {train_raw[0].shape}, {np.array(train_raw[1]).shape}")
print(f"Validation data shape: {val_raw[0].shape}, {np.array(val_raw[1]).shape}")
print(f"Batch size: {args.batch_size}")

if args.mode == "train":
    # Prepare training
    path = os.path.join(
        args.output_dir, "keras_states/" + model.final_name + ".epoch{epoch:02d}.keras"
    )

    metrics_callback = keras_utils.InHospitalMortalityMetrics(
        val_data=(val_raw[0], np.array(val_raw[1])),
    ...
    if not os.path.exists(dirname):
        os.makedirs(dirname)
    saver = ModelCheckpoint(path, verbose=1, save_freq="epoch")
    ...
    model.fit(
        ...
        validation_data=(val_raw[0], np.array(val_raw[1])),

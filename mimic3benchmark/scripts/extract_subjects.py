...
sociodemographic_data = pd.read_csv(
    os.path.join(args.mimic3_path, "sociodemographic_data.csv")
)
stays = stays.merge(
    sociodemographic_data, on=["SUBJECT_ID", "HADM_ID", "ICUSTAY_ID"], how="left"
)

stays = stays.drop([col for col in stays.columns if col.endswith("_y")], axis=1)
stays = stays.rename(
    columns={col: col[:-2] for col in stays.columns if col.endswith("_x")}
)
# Save intermediate file with sociodemographic data
intermediate_path = os.path.join(
    args.output_path, "base_stays_with_sociodemographic_data.csv"
)
